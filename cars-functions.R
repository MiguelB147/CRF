...
WoodSpline <- function(t, dim, degree = 3, eval.penalty = TRUE) {
  ...
  if (eval.penalty) {
    ...
    # for (i in 1:dim) {
    #   for (j in i:dim) {
    #     Q[i,j] <- Q[j,i] <- integrate(int, lower = xl, upper = xu, i, j, derivs = 0, rel.tol = 1e-8)$value
    #     P[i,j] <- P[j,i] <- integrate(int, lower = xl, upper = xu, i, j, derivs = 2, rel.tol = 1e-8)$value
    #   }
    # }
...
EstimatePenal <- function(dim = 10, lambda.init = 10, tol = 0.001, lambda.max = exp(15), step.control = TRUE, type = "bs") { 
  
  tiny <- .Machine$double.eps^0.5
  
  if (type == "bs") {
    model <- WoodSpline(t = mtcars$hp, dim = dim, degree = 3, eval.penalty = T)
    S <- model$S
  } else if (type == "ps") {
    model <- WoodSpline(t = mtcars$hp, dim = dim, degree = 3, eval.penalty = F)
